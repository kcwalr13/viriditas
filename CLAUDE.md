...
- **Cross-platform date picker:** Use `DatePickerField` from `@/components/DatePickerField` for any date input. Metro uses `DatePickerField.web.tsx` for web builds (transparent `<input type="date">` overlay on a styled button — browser's native calendar opens on click) and `DatePickerField.tsx` for native builds (`DateTimePickerAndroid.open()` on Android; inline spinner on iOS). The component always receives and emits YYYY-MM-DD strings. Append `T12:00:00` when constructing a `Date` from a YYYY-MM-DD string to avoid midnight UTC shifting to the previous local day. Never use raw TextInput for date entry.
